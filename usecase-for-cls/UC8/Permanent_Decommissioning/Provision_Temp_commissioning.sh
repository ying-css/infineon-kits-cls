...
#~ echo "set protected update for 0x$CERTIFICATE_OID (Provision for Permanent Decommissioning)"
#~ echo $CERTIFICATE_PER_META | xxd -r -p > cert_per_metadata.bin
#~ echo "Printout cert_per_metadata.bin"
#~ xxd cert_per_metadata.bin
#~ echo "write cert_per_metadata.bin as metadata of 0x$CERTIFICATE_OID"
#~ $EXEPATH/trustm_metadata -w 0x$CERTIFICATE_OID -F cert_per_metadata.bin
...
#~ echo "Step1: Provisioning metadata for 0x$PRIVATE_KEY_OID"
#~ echo "set protected update for 0x$PRIVATE_KEY_OID (Provision for Permanent Decommissioning)"
#~ echo $PRIVATE_KEY_PER_META | xxd -r -p > private_key_per_metadata.bin
#~ echo "Printout private_key_per_metadata.bin"
#~ xxd private_key_per_metadata.bin
#~ echo "write private_key_per_metadata.bin as metadata of 0x$PRIVATE_KEY_OID"
#~ $EXEPATH/trustm_metadata -w 0x$PRIVATE_KEY_OID -F private_key_per_metadata.bin
